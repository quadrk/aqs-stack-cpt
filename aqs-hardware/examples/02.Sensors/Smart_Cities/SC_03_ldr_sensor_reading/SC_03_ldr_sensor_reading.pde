/*  
 *  --[SC_3] - Reading the LDR Sensor on Smart Cities board-- 
 *  
...
void loop()
{
  ...
  // Read the LDR sensor 
  value = SensorCities.readValue(SENS_CITIES_LDR);
  
  // Turn off the sensor
  SensorCities.setSensorMode(SENS_OFF, SENS_CITIES_LDR);
  
  // Part 2: USB printing
  // Print the percentage of luminosity read value through the USB
