...
const SensorCheckbox = ({
  ...
  return (
    ...
    >
      <div className="sensor-chip-icon" style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {payload.map((entry: any, index: number) => {
            ...
            const unit = sensorUnits[sensorId as keyof typeof sensorUnits] || '';
            return (
              <div key={index} style={{ display: 'flex', alignItems: 'center', fontSize: 13 }}>
                <div style={{
                  width: 8, height: 8, backgroundColor: entry.color, marginRight: 8, borderRadius: '50%'
                }} />
                <span>
    ...
    <div className="text-center mb-lg">
      ...
      {deviceId && (
        <div style={{
          ...
          <h3 style={{ fontSize: 16, marginBottom: 10, color: '#ff9300' }}>Debug info</h3>
...
export default function Dashboard() {
  ...
  const deviceOptions = useMemo(() => {
    ...
    if (DEBUG) console.log("deviceOptions:", opts);
    return opts;
  ...
  useEffect(() => {
    ...
    getDevices()
      .then((devices) => {
        setDevices(devices);
        if (DEBUG) console.log("RAW devices:", devices);
        if (devices.length > 0) {
          ...
          if (firstDevice) {
            if (DEBUG) console.log("Init select deviceId:", firstDevice.device_id, "name:", firstDevice.name);
            setSelectedDeviceId(firstDevice.device_id); // UUID only!
          }
        }
  ...
  const handleLocationChange = (location: string) => {
    ...
    if (firstDevice) setSelectedDeviceId(firstDevice.device_id); // UUID
  ...
  const handleDeviceChange = (deviceId: string) => {
    if (DEBUG) console.log("handleDeviceChange value:", deviceId);
    setSelectedDeviceId(deviceId); // always UUID!
    setSelectedSensors([]);
    setIsLoading(true);
  };

  // --- Measurements loading & filtering ---
  useEffect(() => {
    if (!selectedDeviceId) {
      setData([]);
      setIsLoading(false);
    ...
    getMeasurements()
      .then(received => {
        if (DEBUG) {
          ...
          if (received.length > 0) console.log("sample measurement:", received[0]);
        }
